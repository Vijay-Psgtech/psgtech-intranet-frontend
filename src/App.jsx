...
const quickLinks = categories.map((category, index) => ({
  label: category,
  detail: `${feed.filter((item) => item.category === category).length + (index + 2) * 3
    } documents`,
              ...
              <div className="my-6 flex items-center justify-between gap-4 max-lg:flex-col max-lg:items-start">
                ...
                <div className="flex gap-1 overflow-x-auto">
                  {['All updates', 'Administration', 'Academic', 'Student Life'].map((category) => (
                    <button
                      key={category}
                      className={`whitespace-nowrap rounded-md border-0 px-2 py-1.5 text-[10px] ${activeCategory === category
                          ? 'bg-teal-50 text-teal-700'
                          : 'bg-transparent text-slate-500 hover:bg-teal-50 hover:text-teal-700'
                        }`}
